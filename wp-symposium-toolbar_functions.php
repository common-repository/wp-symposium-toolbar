...
function symposium_toolbar_init_globals() {
	...
	global $wp_roles, $wpst_roles_all_incl_visitor, $wpst_roles_all_incl_user, $wpst_roles_all, $wpst_roles_author, $wpst_roles_new_content, $wpst_roles_comment, $wpst_roles_customize, $wpst_roles_updates, $wpst_roles_administrator;
	global $wpst_menus, $wpst_locations;
	
	// Roles
	if ( is_multisite() )
		$wpst_roles_all = $wpst_roles_author = $wpst_roles_new_content = $wpst_roles_comment = $wpst_roles_customize = $wpst_roles_updates = $wpst_roles_administrator = array( 'wpst_superadmin' => __( 'Super Admin' ) );
	else
		$wpst_roles_all = $wpst_roles_author = $wpst_roles_new_content = $wpst_roles_comment = $wpst_roles_customize = $wpst_roles_updates = $wpst_roles_administrator = array();
	...
	if ( $wp_roles->roles ) foreach ( $wp_roles->roles as $key => $role ) {
		...
		if ( isset( $role['capabilities']['edit_theme_options'] ) ) $wpst_roles_customize[$key] = $role_name;
...
function symposium_toolbar_custom_menu_walker( $wp_admin_bar, $custom_menu, $count ) {
	...
	if ( $menu_items ) foreach ( $menu_items as $menu_item ) {
		...
		if ( ! is_array( $menu_item ) ) $menu_item = array();
		...
		$class      = ( isset( $menu_item['classes'] ) )        ? implode( " ", $menu_item['classes'] ) : "";
		$attr_title = ( is_string( $menu_item['attr_title'] ) ) ? $menu_item['attr_title'] : "" ;
		$target     = ( is_string( $menu_item['target'] ) )     ? $menu_item['target'] : "" ;
		$xfn        = ( is_string( $menu_item['xfn'] ) )        ? $menu_item['xfn'] : "" ;
		$meta = array(
			'class' => $class,
			'tabindex' => -1,
			'title' => $attr_title,
